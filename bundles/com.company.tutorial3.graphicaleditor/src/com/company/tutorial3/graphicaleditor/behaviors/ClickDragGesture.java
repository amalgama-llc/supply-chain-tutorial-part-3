...
public class ClickDragGesture extends AbstractGesture {
				...
				if (viewer instanceof InfiniteCanvasViewer) {
					...
					if (creationModel != null) {
						if (creationModel.getSource() != null) {
							creationModel.getSource().invalidateCreatedSegmentFeedback(event.getSceneX(), event.getSceneY());
						}
					}
				}
			}
	...
	private final List<IOnDragHandler> possibleDragPolicies[] = new ArrayList[] {null};
	private EventHandler<MouseEvent> indicationCursorMouseMoveFilter = new EventHandler<MouseEvent>() {
		...
		public void handle(MouseEvent event) {
			...
			if (eventTarget instanceof Node) {
				...
				IViewer viewer = PartUtils.retrieveViewer(getDomain(), target);
				if (viewer != null) {
					possibleDragPolicies[0] = new ArrayList<>(getHandlerResolver().resolve(ClickDragGesture.this, target, viewer, ON_DRAG_POLICY_KEY));
				} else {
				...
				// cursor
				ListIterator<? extends IOnDragHandler> dragIterator = possibleDragPolicies[0].listIterator(possibleDragPolicies[0].size());
